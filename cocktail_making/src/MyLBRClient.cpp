...
MyLBRClient::MyLBRClient(double freqHz, double amplitude)
{
    ...
    // Read the Data
    pos_data = readCSV( "/home/baxterplayground/Documents/DMP_combinations/data/csv/cocktail_pos.csv" );
    ...
    // Read the Data
    R_data_osc = readCSV( "/home/baxterplayground/Documents/DMP_combinations/data/csv/cocktail_orient_rhyth.csv" );

    // Number of data points, orientation
    N_orient_rhyth  = R_data_osc.cols( )/3;
    N_curr_orient_r = 0 ;

    // Read the Data
    R_data_pour = readCSV( "/home/baxterplayground/Documents/DMP_combinations/data/csv/cocktail_orient_pour.csv" );

    // Number of data points, orientation
    N_orient_pour  = R_data_pour.cols( )/3;
    N_curr_orient_d = 0 ;

    std::cout << "Data Length for Orientation, Rhythmic: " << N_orient_rhyth    << std::endl;
...
void MyLBRClient::command()
{
    ...
    // If over some time, go through the
    if( is_pressed && !is_pos_done )
    {
        ...
        // Read the orientation for oscillation
        if( t - t_pressed >= 12 )
        {
            if( N_curr_orient_r >= N_orient_rhyth-1 )
            {
                N_curr_orient_r = 0;
            }

            R_des = R_des * R_data_osc.block< 3, 3 >( 0, 3*N_curr_orient_r );
            N_curr_orient_r += 2;
        }
    ...
    if( is_pos_done )
    {
        ...
        if( N_curr_orient_r >= N_orient_rhyth-1 && N_rot_count < 4 )
        {
            N_curr_orient_r = 0;
            N_rot_count++;
        }
        ...
        {
            N_curr_orient_r = 0;
            R_des = R_des * R_data_pour.block< 3, 3 >( 0, 3*N_curr_orient_d );
            N_curr_orient_d += 2;

            if( N_curr_orient_d >= N_orient_pour-1 )
            {
                N_curr_orient_d = N_orient_pour-1;
            }
        }
    ...
    if (  ( n_step % 5 ) == 0 && is_pressed )
    {
        ...
        f << " Virtual Traj. " << p0.transpose( ).format( fmt ) ;
        f << std::endl;
