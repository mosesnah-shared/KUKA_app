...
bool is_SO3(const Eigen::Matrix3d& R) {
    double det = R.determinant();
    return (det > 0.999 && det < 1.001) && (R.transpose() * R).isApprox(Eigen::Matrix3d::Identity(), 1e-6);
}

Eigen::Vector4d SO3_to_quat(const Eigen::Matrix3d& R) {
    assert(is_SO3(R) && "Input matrix is not a valid SO(3) matrix");
    ...
    Eigen::Vector4d tmp_cmp( R00, R(0, 0), R(1, 1), R(2, 2) );
    k = 0;
    for (int i = 1; i < 4; ++i) {
        if (std::abs(tmp_cmp[i]) > std::abs(tmp_cmp[k])) {
            k = i;
        }
    }

    if (k == 0) {
        ek = 0.5 * std::sqrt(1 + R00);
    } else {
        ek = 0.5 * std::sqrt(1 + 2 * R(k-1, k-1) - R00);
    }

    switch (k) {
    case 0:
        quat(0) = ek;
        quat(1) = 0.25 * (R(2, 1) - R(1, 2)) / ek;
        quat(2) = 0.25 * (R(0, 2) - R(2, 0)) / ek;
        quat(3) = 0.25 * (R(1, 0) - R(0, 1)) / ek;
        break;
    case 1:
        quat(0) = 0.25 * (R(2, 1) - R(1, 2)) / ek;
        quat(1) = ek;
        quat(2) = 0.25 * (R(1, 0) + R(0, 1)) / ek;
        quat(3) = 0.25 * (R(0, 2) + R(2, 0)) / ek;
        break;
    case 2:
        quat(0) = 0.25 * (R(0, 2) - R(2, 0)) / ek;
        quat(1) = 0.25 * (R(1, 0) + R(0, 1)) / ek;
        quat(2) = ek;
        quat(3) = 0.25 * (R(2, 1) + R(1, 2)) / ek;
        break;
    case 3:
        quat(0) = 0.25 * (R(1, 0) - R(0, 1)) / ek;
        quat(1) = 0.25 * (R(0, 2) + R(2, 0)) / ek;
        quat(2) = 0.25 * (R(2, 1) + R(1, 2)) / ek;
        quat(3) = ek;
        break;
    default:
        assert(false && "Unexpected case in quaternion calculation");
        break;
    }
...
MyLBRClient::MyLBRClient(double freqHz, double amplitude)
{
    ...
    offset = Eigen::Vector3d( 0.00, 0.0, 0.0 );
    ...
    Kp = 200 * Eigen::Matrix3d::Identity( );
    Bp =  20 * Eigen::Matrix3d::Identity( );

    Kq = 6.0 * Eigen::MatrixXd::Identity( myLBR->nq, myLBR->nq );
    Bq = 4.5 * Eigen::MatrixXd::Identity( myLBR->nq, myLBR->nq );

    Keps = 50.0 * Eigen::Matrix3d::Identity( );
    Beps =  5.0 * Eigen::Matrix3d::Identity( );
    Emat =        Eigen::Matrix3d::Zero( );

    // Initialization of Quaternions
    eta = 0.0;
    eps = Eigen::Vector3d::Zero( 3 );
    ...
    printf( "The current script maintains the current orientation with type2, orientation \n" );
}
...
void MyLBRClient::command()
{
    ...
    J  = myLBR->getBodyJacobian( q, 7, TYPE_JOINT );
    ...
    quat_del = SO3_to_quat( R_del );

    // The euler Parameters
    eta = quat_del( 0 );
    eps = quat_del.tail<3>();
    Emat = eta * Eigen::Matrix3d::Identity( ) - R3_to_so3( eps );

    tau_imp1 = Jp.transpose( ) * ( Kp * ( p_init - p_curr ) + Bp * ( - dp_curr ) );
    tau_imp2 = Kq * ( q0_init - q ) + Bq * ( -dq );
    tau_imp3 = Jr.transpose( ) * ( 2 * Emat.transpose( ) * Keps * eps + Beps * Jr * ( - dq ) );
