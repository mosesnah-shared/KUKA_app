...
MyLBRClient::MyLBRClient(double freqHz, double amplitude)
{
    ...
    delx1 = Eigen::Vector3d(  0.45, 0.0,  1.3 );
    delz1 = Eigen::Vector3d(  0.00, 0.0, -1.4 );

    mjt1  = new MinimumJerkTrajectory( 3,                              p0i,    p0i + delx1, 2*D, ti                );        // Forward
    mjt2  = new MinimumJerkTrajectory( 3, Eigen::Vector3d( 0.0, 0.0, 0.0 ),          delz1, 1.3, ti + 1 * ( D + toff ) );    // Backward
    ...
    Kp = 1000 * Eigen::MatrixXd::Identity( 3, 3 );
